...
function updateUserInterface() {
    if (currentUser) {
        ...
        // Show admin panel button for admins and founder
        if (currentUser.rank === 'admin' || currentUser.rank === 'founder') {
...
function promoteUser(newRank) {
    ...
    if (['helper', 'moderator', 'admin', 'sponsor'].includes(newRank.toLowerCase())) {
...
window.addEventListener('error', function(e) {
    if (e.target.tagName === 'IMG') {
        e.target.src = 'data:image/svg+xml;base64,';
    }
});
    ...
    if (addStaffForm) {
        addStaffForm.addEventListener('submit', function(e) {
            ...
            if (!usersDatabase[websiteUsername]) {
                ...
                usersDatabase[websiteUsername].rank = rank;
            ...
            showNotification(`${websiteUsername} added as ${rank}!`, 'success');
            this.reset();
            loadCurrentStaff();
            updateStaffDisplay();
        });
...
function openAnnouncementManager() {
    if (currentUser && (currentUser.rank === 'admin' || currentUser.rank === 'founder' || currentUser.rank === 'moderator')) {
