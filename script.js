...
// Initialize databases - will be loaded async
let usersDatabase = {
    'Coen3111': {
        username: 'Coen3111',
...
async function loadAllDatabases() {
    try {
        const loadedUsers = await loadFromStorage('usersDatabase');
        if (loadedUsers) {
            usersDatabase = loadedUsers;
        }
        
        const loadedStaff = await loadFromStorage('staffDatabase');
        if (loadedStaff) {
            staffDatabase = loadedStaff;
        }
        
        const loadedAnnouncements = await loadFromStorage('announcementsDatabase');
        if (loadedAnnouncements) {
            announcementsDatabase = loadedAnnouncements;
        }
        
        const loadedGallery = await loadFromStorage('galleryDatabase');
        if (loadedGallery) {
            galleryDatabase = loadedGallery;
        }
        
        console.log('All databases loaded from files');
        updateStaffDisplay();
...
function saveToStorage(key, data) {
    try {
        ...
        // Use fetch to save data to server
        fetch(`/api/save/${key}`, {
            method: 'POST',
            headers: {
